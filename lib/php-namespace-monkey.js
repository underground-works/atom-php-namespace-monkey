...
export default {
    ...
    addBoilerplate(buffer) {
        ...
        fs.stat(buffer.file.path, (err, stats) => {
            ...
            let type = this.resolveType(className)
    ...
    resolveType(className) {
        let resolvedType = 'class';

        className.replace(/(Interface|Trait|Abstract)$/, function(match, type)
        {
          resolvedType = type.toLowerCase();

          if ('abstract' === resolvedType)
          {
            resolvedType += ' class'
          }
        })
